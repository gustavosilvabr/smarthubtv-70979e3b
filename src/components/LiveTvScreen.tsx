...
import { PlayerDiagnosticsOverlay } from "@/components/PlayerDiagnosticsOverlay";
...
export function LiveTvScreen({
  ...
  useEffect(() => {
    ...
    if (raw) {
      try {
        ...
        // Ignore invalid data from an older session.
      }
    }
  }, []);
  ...
  const {
    ...
    retry,
    diagnostics,
  } = useHlsPlayer(videoRef, selected, { lowQuality });
  ...
  useEffect(() => {
    const handleFullscreenChange = () => {
      const isCurrentlyFullscreen = document.fullscreenElement === playerWrapRef.current;
  ...
  return (
    <div
      ref={containerRef}
      className="relative flex min-h-screen flex-col overflow-x-hidden overflow-y-auto bg-[radial-gradient(ellipse_at_top,_rgba(88,28,135,0.35)_0%,_#0a0613_55%,_#050308_100%)] text-foreground sm:h-screen sm:overflow-hidden"
    >
      ...
      <div className="relative z-10 grid min-h-0 flex-none grid-cols-1 gap-2 p-2 sm:flex-1 sm:grid-cols-[200px_240px_minmax(0,1fr)] sm:grid-rows-1 sm:gap-3 sm:p-3 md:grid-cols-[230px_280px_minmax(0,1fr)] md:gap-4 md:p-4 xl:grid-cols-[260px_320px_minmax(0,1fr)]">
        <aside
          ref={sidebarRef}
        ...
        <section className="flex min-h-0 flex-col gap-2 sm:gap-3 overflow-hidden order-2 col-span-1 sm:col-span-auto row-start-1 sm:row-start-auto">
          ...
          >
            ...
            <button
              ...
              className="absolute right-2 top-2 z-30 rounded-full bg-black/70 p-2 text-white opacity-80 ring-1 ring-white/15 transition hover:bg-black/90 hover:opacity-100 focus:outline-none focus-visible:ring-2 focus-visible:ring-amber-400 sm:right-3 sm:top-3"
            ...
            <button
              type="button"
              onClick={toggleSound}
              className="absolute left-2 top-2 z-30 inline-flex items-center gap-2 rounded-full bg-black/70 px-3 py-2 text-xs font-semibold text-white ring-1 ring-white/15 transition hover:bg-black/90 focus:outline-none focus-visible:ring-2 focus-visible:ring-amber-400 sm:left-3 sm:top-3"
            ...
            {selected && (loading || bufferStatus) && !error && (
              <div className="absolute inset-0 z-20 grid place-items-center bg-black/70 text-white">
            ...
            {selected && !loading && error && (
              <div
                ...
                className="absolute inset-0 z-20 grid place-items-center bg-black/85 p-6 text-center text-white"
              >
            ...
            {selected && (
              <div
                className={[
                  "pointer-events-none absolute inset-x-0 bottom-0 z-10 w-full px-4 py-3 text-white transition-opacity duration-300",
                  isFullscreen
                    ? "bg-gradient-to-t from-black/95 via-black/80 to-transparent sm:px-8 sm:py-6"
                    : "bg-black/65 backdrop-blur-sm",
                  isFullscreen && !showFullscreenControls ? "opacity-0" : "opacity-100",
                ].join(" ")}
              >
                <div
                  className={
                    isFullscreen
                      ? "text-2xl font-bold tracking-wide sm:text-3xl md:text-4xl"
                ...
                <div
                  className={
                    isFullscreen
                      ? "mt-2 text-sm font-semibold text-amber-300/90 sm:text-base"
                      : "mt-0.5 truncate text-xs text-white/65"
                  }
                >
            ...
            {isFullscreen && (
              <div
                className={`pointer-events-none absolute right-4 z-20 sm:right-6 inset-y-0 flex flex-col items-center justify-center gap-6 transition-opacity duration-300 ${
            ...
            {showDiagnostics && (
              <PlayerDiagnosticsOverlay
                ...
                onToggleLowQuality={() => setLowQuality((value) => !value)}
          ...
          {selected && (
            <div className="flex flex-col gap-2 rounded-2xl border border-white/10 bg-[#140a24]/80 px-3 py-2 backdrop-blur sm:px-4 sm:py-3">
